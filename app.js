...
// ─── Onboarding ───────────────────────────────────────────────────────────────
// Les données des suggestions sont dans suggestions.js (chargé avant app.js)
// SUGGESTIONS est donc disponible globalement ici.
...
// ─── Conseil du jour ──────────────────────────────────────────────────────────
// Retourne un conseil rotatif basé sur la date + l'id de l'objectif
// → conseil différent chaque jour, mais stable dans la journée
function getTip(obj) {
  if (!obj.suggestionKey) return null;
  const sugg = SUGGESTIONS.find(s => s.key === obj.suggestionKey);
  if (!sugg || !sugg.tips?.length) return null;
  // Index basé sur la date du jour + hash de l'id → stable dans la journée
  const dayNum = Math.floor(Date.now() / 86400000);
  const hash   = obj.id.split('').reduce((a, c) => a + c.charCodeAt(0), 0);
  const idx    = (dayNum + hash) % sugg.tips.length;
  return sugg.tips[idx];
}
      ...
      ${tip ? `
        <div class="tip-block">
          ...
          <div class="tip-content">
            <p class="tip-msg">${esc(tip.msg)}</p>
            ${tip.url ? `<a href="${tip.url}" target="_blank" rel="noopener" class="tip-link">En savoir plus →</a>` : ''}
          </div>
        </div>` : ''}
    </article>`;
}
